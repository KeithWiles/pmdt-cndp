...
func ProcessPanelSetup(nextSlide func()) (pageName string, content tview.Primitive) {
	...
	to := tab.New(processPanelName, perfmon.app)
	pg.tabOrder = to
	...
	perfmon.timers.Add(processPanelName, func(step int, ticks uint64) {
...
func (pg *ProcessPanel) displayProcessData(table *tview.Table) {
	...
	for _, p := range pg.procs {
		...
		// If the percent load is greater then 95% it is most likely a DPDK process
...
func (pg *ProcessPanel) displayProcess2Info(table *tview.Table) {
	...
	if err == nil {
		...
		for _, v := range rlimit {
			...
			SetCell(table, row, 1, cz.LightBlue(FormatUnits(uint64(v.Soft)), 8))
			SetCell(table, row, 2, cz.LightBlue(FormatUnits(uint64(v.Hard)), 8))
			SetCell(table, row, 3, cz.LightBlue(FormatUnits(uint64(v.Used)), 8))
			row++
		}
	}
	row++
	...
	if err == nil {
		...
		row++
		SetCell(table, row, 1, cz.LightBlue(FormatBytes(mem.RSS)))
		SetCell(table, row, 2, cz.LightBlue(FormatBytes(mem.VMS)))
		SetCell(table, row, 3, cz.LightBlue(FormatBytes(mem.HWM)))
		SetCell(table, row, 4, cz.LightBlue(FormatBytes(mem.Data)))
		row++
		...
		row++
		SetCell(table, row, 1, cz.LightBlue(FormatBytes(mem.Stack)))
		SetCell(table, row, 2, cz.LightBlue(FormatBytes(mem.Locked)))
		SetCell(table, row, 3, cz.LightBlue(FormatBytes(mem.Swap)))
		row++
	}
}
