...
func SysInfoPanelSetup(nextSlide func()) (pageName string, content tview.Primitive) {
	...
	to := tab.New(sysinfoPanelName, perfmon.app)
	pg.tabOrder = to
	...
	// Setup the static PBF data for display
	pbf.CheckForDrivers()
	pbf.PStates()
	pbf.CStates()
	...
	perfmon.timers.Add(sysinfoPanelName, func(step int, ticks uint64) {
...
func (pg *PageSysInfo) displayCPU(view *tview.TextView) {
	...
	str += fmt.Sprintf("P-State driver : [%s]   ", cz.SkyBlue(pbf.Driver))
	str += fmt.Sprintf("CPU Base Frequency: %s MHz\n", cz.Red(pbf.FreqP1))
	str += fmt.Sprintf("P-States       : %s\n", cz.SkyBlue(pbf.Freqs))

	if len(pbf.Freqs) > 0 && (pbf.Freqs[0]-1) == pbf.Freqs[1] {
		str += fmt.Sprintf("Turbo Available: %s (use pstate '%s') ",
			cz.LightGreen("Yes"),
			cz.Orange(pbf.Freqs[0]))
	} else if len(pbf.Freqs) > 0 && pbf.FreqP0 >= pbf.FreqP1 {
		str += fmt.Sprintf("Turbo Available: %s (any pstate above '%s') ",
			cz.LightGreen("Yes"),
			cz.Orange(pbf.FreqP1))
	} else {
		str += fmt.Sprintf("Turbo Available: %s ", cz.Red("No"))
	}
	str += fmt.Sprintf("Governors: %s ", cz.SkyBlue(pbf.Governors()))
	str += fmt.Sprintf("C-States : %s\n", cz.SkyBlue(pbf.CStates()))
