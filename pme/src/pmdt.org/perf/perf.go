...
func (ev *Event) Measure(f func()) (Count, error) {
	if err := ev.MeasureStart(); err != nil {
		return Count{}, err
	}

	f()

	return ev.MeasureEnd()
}

// MeasureStart disables the event, resets it, enables it
func (ev *Event) MeasureStart() error {
...
// MeasureEnd disables the event,
// then reads the Count associated with the event.
func (ev *Event) MeasureEnd() (Count, error) {
...
func (ev *Event) MeasureGroup(f func()) (GroupCount, error) {
	if err:= ev.MeasureGroupStart(); err != nil {
		return GroupCount{}, err
	}

	f()

	return ev.MeasureGroupEnd()
}

// MeasureGroupStart disables the event, resets it, enables it
func (ev *Event) MeasureGroupStart() error {
	if err := ev.Disable(); err != nil {
		return err
	}
	if err := ev.Reset(); err != nil {
...
// MeasureGroupEnd disables the event,
// then reads the Count associated with the event.
func (ev *Event) MeasureGroupEnd() (GroupCount, error) {
...
// AddHardwareLabel - Add a new label to the hardware counters
func AddHardwareLabel(name, alias string, eventNum uint64) {
	...
	hardwareLabels[HardwareCounter(eventNum)] = hw
}
