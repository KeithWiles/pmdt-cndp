...
// PrintCount prints a table of Values to w.
func (c Count) PrintCount(w io.Writer) error {
	ew := &errWriter{w: w}
	...
	if c.ID != 0 {
		fmt.Fprintf(tw, "%s\t%12d %d", c.Label, c.Value, c.ID)
	} else {
		fmt.Fprintf(tw, "%s\t%12d", c.Label, c.Value)
	}
